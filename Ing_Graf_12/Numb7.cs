...
{
    public partial class Numb7 : Form
    {
        ...
        public double RotateObject(double Theta, double A, double B, double C, double u, double v, double w, double x, double y, double z, ref double NewX, ref double NewY)
        {
            ...
            + (-A * Lu + u * (B * v + C * w - v * y - w * z) + Lu * x) *
            Math.Cos(Theta) + SqrL * (-C * v + B * w - w * y + v * z) *
            Math.Sin(Theta)) / L;
            NewY = (B * Lv + v * (-A * u - C * w + u * x + v * y + w * z)
            + (-B * Lv + v * (A * u + C * w - u * x - w * z) + Lv * y) *
            Math.Cos(Theta) + SqrL * (-C * u - A * w + w * x - u * z) *
            Math.Sin(Theta)) / L;
            NewZ = (C * Lw + w * (-A * u - B * v + u * x + v * y + w * z)
            + (-C * Lw + w * (A * u + B * v - u * x - v * y) + Lw * z) *
            Math.Cos(Theta) + SqrL * (-B * u + A * v - v * x + u * y) *
            Math.Sin(Theta)) / L;
       ...
       y[0], z[0], ref newX[0], ref newY[0]);
        ...
        private void FormLab7_Load(object sender, EventArgs e)
        {
