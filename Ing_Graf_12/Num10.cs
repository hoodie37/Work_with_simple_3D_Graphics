    ...
    public partial class Num10 : Form
    {
        ...
        private void DrawShape(PaintEventArgs e)
        {
            Matrix matrix;
                    ...
                    Pen[] pens = new Pen[] { new Pen(Color.Blue, 2), new Pen(Color.Red, 2), new Pen(Color.Black, 2), new Pen(Color.Purple, 2), new Pen(Color.Orchid, 2), new Pen(Color.Green, 2) };
                    for (int i = 0; i < SurFaceList.Count; i++)
                    {
                        SurFace tempx, tempnewx;
                        tempx = (SurFace)SurFaceList[i];
                        tempnewx = RotateSurFace(X, Y, Z, tempx);
                        PointF[] points = new PointF[] { new PointF(Convert.ToInt32(tempnewx.x1.x), Convert.ToInt32(tempnewx.x1.y)), new PointF(Convert.ToInt32(tempnewx.x2.x), Convert.ToInt32(tempnewx.x2.y)), new PointF(Convert.ToInt32(tempnewx.x3.x), Convert.ToInt32(tempnewx.x3.y)), new PointF(Convert.ToInt32(tempnewx.x4.x), Convert.ToInt32(tempnewx.x4.y)) };
                        e.Graphics.DrawPolygon(pens[i], points);
                        //e.Graphics.FillPolygon(solids[i], points, FillMode.Winding);
                    }
                    int Xstart, Ystart;
